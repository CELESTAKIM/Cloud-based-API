...
/**
 * Utility to generate a random hex color for the county visualization.
 */
...
app.get("/api/counties/geojson", async (req, res) => {
  try {
    ...
    const collectionWithProps = counties.map((feature) => {
        ...
        return feature
          .set('bbox', bbox)
          .set('color', generateRandomColor()); // Assign a random color
    });
      ...
      case "ndvi":
        ...
        legendInfo = {
          ...
          classes: [
            ...
            { color: "#4a864d", label: "Dense Vegetation (> 0.6)" }
          ],
        };
        break;
